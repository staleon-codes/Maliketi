<?php
/**
 * Motta maintenance functions and definitions.
 *
...
class Maintenance {
	...
	public function maintenance_redirect() {
		...
		if ( ! $page_id || ! $page_url ) {
			if ( 'coming_soon' == $mode ) {
				$message = sprintf( '<h1>%s</h1><p>%s</p>', esc_html__( 'Coming Soon', 'motta' ), esc_html__( 'Our website is under construction. We will be here soon with our new awesome site.', 'motta' ) );
			} else {
				$message = sprintf( '<h1>%s</h1><p>%s</p>', esc_html__( 'Website Under Maintenance', 'motta' ), esc_html__( 'Our website is currently undergoing scheduled maintenance. Please check back soon.', 'motta' ) );
			}
		...
		if ( ! is_page( $page_id ) ) {
			...
			add_filter( 'motta_get_popup', '__return_false' );
